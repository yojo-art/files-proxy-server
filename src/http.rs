...
pub(crate) async fn http_server(send_queue: Arc<AgentJobManager>, config: Arc<ConfigFile>) {
	...
	let app = Router::new()
		.route(
			"/",
			get(|| async move { axum::http::StatusCode::NO_CONTENT }),
		)
		.route(
			"/ping",
			get(|| async move {
				let (ms, ok) = send_queue0.agent_ping().await;
				let json = PingStatus { ms, ok };
		...
		.route(
			"/*path",
			get(|path, headers| get_file(path, headers, client, send_queue, redis_client, config)),
		);
...
async fn get_file(
	...
	(
		...
		axum::headers::HeaderMap,
		StreamBody<impl futures::Stream<Item = Result<axum::body::Bytes, reqwest::Error>>>,
	),
...
> {
	...
	if json.is_none() {
		...
		if let Some(redis_client) = redis_client.as_ref() {
			...
			if let Some(res_json) = res_json.as_ref() {
				let res = match config.redis_ttl {
					Some(ttl_seconds) => redis_client.set_ex(&path, res_json, ttl_seconds),
					None => redis_client.set(&path, res_json),
				};
				match res {
					Ok(v) => {
						let _: () = v;
		...
		if res.status == 404 {
			...
			resp.headers_mut()
				.append("X-AgentStatus", res.status.to_string().parse().unwrap());
			return Err(resp);
		}
		if res.status != 200 {
	...
	if type_id == 2 {
		headers.append(
			"Location",
			format!(
				"{}static.webp?url={}&static=1",
				config.media_proxy_url,
	...
	let mut request = match request {
		Ok(request) => request,
		Err(e) => {
			return Err((axum::http::StatusCode::BAD_GATEWAY, format!("{:?}", e)).into_response())
		}
	};
	...
	fn add_remote_header(
		key: &'static str,
		headers: &mut axum::headers::HeaderMap,
